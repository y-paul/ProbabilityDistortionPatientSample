...
transformed parameters {
  ...
  for (i in 1:n_pat) {
    ...
    // DESCRIPTION (choices_pat[,1])
    {
      ...
      real wA1 = pwf(pa_pat[i,1], 1);
      real wA2 = pwf(pa_pat[i,2], 1);
      real svA = vA1 * wA1 + vA2 * wA2;

      real vB1 = xb_pat[i,1];
      real vB2 = - lambda_de_pat[s] * xb_pat[i,2];
      real wB1 = pwf(pb_pat[i,1], 1);
      real wB2 = pwf(pb_pat[i,2], 1);
    ...
    // EXPERIENCE (choices_pat[,2])
    {
      ...
      real wA1 = pwf(fa_pat[i,1], 1);
      real wA2 = pwf(fa_pat[i,2], 1);
      real svA = vA1 * wA1 + vA2 * wA2;

      real vB1 = xb_pat[i,1];
      real vB2 = - lambda_ex_pat[s] * xb_pat[i,2];
      real wB1 = pwf(fb_pat[i,1], 1);
      real wB2 = pwf(fb_pat[i,2], 1);
      real svB = vB1 * wB1 + vB2 * wB2;
  ...
  for (i in 1:n_con) {
    ...
    // DESCRIPTION (choices_con[,1])
    {
      ...
      real wA1 = pwf(pa_con[i,1], 1);
      real wA2 = pwf(pa_con[i,2], 1);
      real svA = vA1 * wA1 + vA2 * wA2;

      real vB1 = xb_con[i,1];
      real vB2 = - lambda_de_con[s] * xb_con[i,2];
      real wB1 = pwf(pb_con[i,1], 1);
      real wB2 = pwf(pb_con[i,2], 1);
      real svB = vB1 * wB1 + vB2 * wB2;
    ...
    // EXPERIENCE (choices_con[,2])
    {
      ...
      real wA1 = pwf(fa_con[i,1], 1);
      real wA2 = pwf(fa_con[i,2], 1);
      real svA = vA1 * wA1 + vA2 * wA2;

      real vB1 = xb_con[i,1];
      real vB2 = - lambda_ex_con[s] * xb_con[i,2];
      real wB1 = pwf(fb_con[i,1], 1);
      real wB2 = pwf(fb_con[i,2], 1);
      real svB = vB1 * wB1 + vB2 * wB2;
